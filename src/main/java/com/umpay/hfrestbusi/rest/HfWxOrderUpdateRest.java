...
/** ******************  类说明  *********************
 * class       :  HfWxOrderUpdateRest
 * @author     :  chenwei
 * @version    :  1.0  
 * description :  更新无线商户订单
 * @see        :                        
 * ************************************************/   
public class HfWxOrderUpdateRest extends BaseRest {
	...
	public String doUpdateService(Map<String, String> urlargs, String id) throws Exception {
		...
		updateMap.put(HFBusiDict.MOBILEID+".string", mobileid);
		updateMap.put(HFBusiDict.ORDERSTATE+".int", orderstate);
		updateMap.put("_tablename",tableNum);
 		int num = dal.update("psql_Wxorder.modOrder", updateMap);
