 ...
 * @see        :                        
 * ************************************************/   
public class UpdateTransRest extends BaseRest {
...
//		newTransAllMap.put(HFBusiDict.MERCHECK+".int",mercheck);
	...
	throws Exception {
		...
		String servpriv = StringUtil.trim(urlargs.get("servpriv"));
		String month = transSeq.substring(transSeq.length()-4);
		
		
		Map<String,String> map = new HashMap<String,String>();
		map.put(HFBusiDict.BANKTRACE+".string",banktrace);
		map.put(HFBusiDict.FUNCODE+".string",funcode);
		map.put(HFBusiDict.TRANSEQ+".string",transSeq);
		map.put("servpriv.string",servpriv);
		map.put("_tablename",month);
		
		int state = dal.update("psql_UPTRANSYYYYMM.UPtransBytranSeq", map);
		if(state!=1){
